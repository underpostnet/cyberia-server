// Package engine_client — grpc_client.go
//
// GrpcClient wraps the generated CyberiaDataServiceClient with
...
// GrpcClient is a high-level wrapper around the generated gRPC client.
...
// NewGrpcClient dials the Engine gRPC server at address (host:port) and returns
// a connected GrpcClient. The address is required (resolved by package config).
...
func (c *GrpcClient) FetchObjectLayer(ctx context.Context, itemID string) (*game.ObjectLayer, error) {
	ctx, cancel := context.WithTimeout(ctx, callTimeout)
	defer cancel()
...
func (c *GrpcClient) FetchFullInstance(ctx context.Context, instanceCode string) (*pb.GetFullInstanceResponse, error) {
