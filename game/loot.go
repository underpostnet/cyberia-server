...
// │     (drop_collect) to every viewer     , and removes the token. A kill  │
...
const (
	...
	// animation length (sent as launchMs in drop_spawn).
...
func (s *GameServer) broadcastDropSpawn(mapState *MapState, drop *BotState, origin, landing Point) {
	payload := DropSpawn{
		...
		LandingX: landing.X,
		LandingY: landing.Y,
		LaunchMs: int(lootLandingWindow / time.Millisecond),
	...
	for _, player := range mapState.players {
		if rectsOverlap(player.AOI, originRect) {
			sendMessage(player, "drop_spawn", payload)
		}
	}
}
...
func (s *GameServer) broadcastDropCollect(mapState *MapState, drop *BotState, collectorID string, cx, cy float64) {
	...
	for _, player := range mapState.players {
		if player.ID == collectorID || rectsOverlap(player.AOI, dropRect) {
			sendMessage(player, "drop_collect", payload)
		}
	}
}
